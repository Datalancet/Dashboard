...
const ScorecardDataTable = ({ onDataChange,chartType}) => {
  ...
  const [publishedImageURLInMemory, setPublishedImageURLInMemory] = useState(null);
  ...
  const updateAPIDataFileRef = useRef(null);
...
useEffect(() => {
  if (projectId) {
    const storedPublishState = localStorage.getItem(`isPublished_${projectId}`);
    setIsPublished(storedPublishState === 'true');
  }
}, [projectId]);
...
const fetchDataFromAPI = async () => {
  ...
  try {
    ...
    if (projectData.data_file) {
      ...
      setHeaders(defaultData[0]);
      setTableData(defaultData.slice(1));
    }

    // Check if the project is published
    const isPublished = localStorage.getItem(`publishedProject_${projectId}`) === 'true';
    setIsPublished(isPublished);

    if (isPublished) {
      // Fetch and set embed URLs if the project is published
      const embedURL = `http://dashboardtool.pythonanywhere.com/embed/${projectId}`;
      const scriptURL = `http://dashboardtool.pythonanywhere.com/script/${projectId}`;
...
const handleRowSelection = useCallback((rowIndex: number) => {
  ...
  setSelectedRows(prev => {
    ...
    // Update API data with debounce
    const timer = setTimeout(() => {
      if (updateAPIDataFileRef.current) {
        updateAPIDataFileRef.current(newSelection).then(() => {
...
useEffect(() => {
  if (projectId) {
    ...
    const storedSelectedRows = localStorage.getItem(`selectedRows_${projectId}`);
    
    if (storedData && storedHeaders && storedSelectedRows) {
      const allData = JSON.parse(storedData);
      const selectedRowIndices = JSON.parse(storedSelectedRows);
      const selectedData = allData.filter((_, index) => selectedRowIndices.includes(index));
...
useEffect(() => {
  if (projectId) {
    const storedEmbedURL = sessionStorage.getItem(`embedURL_${projectId}`);
    const storedScriptURL = sessionStorage.getItem(`scriptURL_${projectId}`);
  ...
  const fetchProjectDetails = async () => {
    try {
      ...
      if (projectData && projectData.project_data) {
        ...
        // Check localStorage for published state
        ...
        if (isPublishedInLocalStorage) {
          // Retrieve stored embed URL and script URL
          ...
          if (storedEmbedURL && storedScriptURL) {
            ...
            // If not in localStorage but available in project data, use and store it
            ...
            console.warn('No embed URL found, generating default');
          ...
          const imageAvailable = localStorage.getItem(`imageAvailable_${projectId}`) === 'true';
          if (imageAvailable) {
            console.log('Image is available for this project');
            // We don't set the actual image URL here, as we'll regenerate it when needed
          }
        }
        ...
        if (storedTableData && storedHeaders && storedSelectedRows) {
          ...
          setSelectedRows(parsedSelectedRows);
  
          // Update the data in the parent component if needed
          if (onDataChange) {
            const selectedData = parsedSelectedRows.length > 0
              ? parsedTableData.filter((_, index) => parsedSelectedRows.includes(index))
              : parsedTableData;
            onDataChange(parsedHeaders, selectedData);
          }
        ...
        // Set other project details if needed
        // For example:
        // setProjectName(projectData.project_data.name);
        // setProjectDescription(projectData.project_data.description);
  ...
  const handlePublish = async () => {
    setIsModalOpen(false);
    setIsPublishing(true);  // Start the publishing effect
    try {
      const scorecardElement = document.getElementById("scorecard-for-publish");
      if (!scorecardElement) {
      ...
      if (projectId) {
        ...
        if (updateResult && updateResult.name && updateResult.description) {
          ...
          if (updateResult.project_status === "Published") {
            ...
            // Store publish state and URLs in localStorage
            localStorage.setItem(`isPublished_${projectId}`, 'true');
            localStorage.setItem(`embedURL_${projectId}`, fullEmbedURL);
            localStorage.setItem(`scriptURL_${projectId}`, fullScriptURL);
            
            // Store only the project ID and published status
            localStorage.setItem(`publishedProject_${projectId}`, 'true');
            
            // Instead of storing the image, just set a flag that an image is available
            localStorage.setItem(`imageAvailable_${projectId}`, 'true');
            
            // Remove any previously stored data for this project
            localStorage.removeItem(`tableData_${projectId}`);
            localStorage.removeItem(`headers_${projectId}`);
            localStorage.removeItem(`selectedRows_${projectId}`);
            
            // Set the published image URL for the current session only
            setPublishedImageURL(imageDataURL);
            setPublishedImageURLInMemory(imageDataURL);
        ...
        // Handle case where there's no projectId
        alert("Cannot save project without a project ID. Please ensure you're working with a valid project.");
      }
    } catch (error) {
      console.error('Error publishing project:', error);
      alert(`Failed to publish project: ${error.message}. Please check the console for more details.`);
    } finally {
      setIsPublishing(false);  // End the publishing effect, regardless of success or failure
    }
  };

  const handleDownloadImage = async () => {
    try {
      let imageToDownload = publishedImageURL;
      
      if (!imageToDownload && localStorage.getItem(`imageAvailable_${projectId}`) === 'true') {
      ...
      alert(`Failed to download image: ${error.message}. Please try republishing the project.`);
    }
  };
  ...
  useEffect(() => {
    updateAPIDataFileRef.current = async (selectedRows) => {
      if (selectedRows.length === 0) return;

      const selectedData = [headers, ...selectedRows.map(index => tableData[index])];
      ...
      try {
        ...
        const formData = new FormData();
        formData.append('id', projectId);
        formData.append('name', projectData.project_data.name);
        formData.append('description', projectData.project_data.description);
        formData.append('html_file', new Blob([atob(projectData.html_file)], { type: 'text/html' }), '/demo.html');
        formData.append('data_file', new Blob([csvContent], { type: 'text/csv' }), 'data.csv');
        formData.append('project_status', 'Draft');

        const updateResponse = await fetch('https://dashboardtool.pythonanywhere.com/api/v1/projects/create-or-upload/', {
          method: 'POST',
          body: formData,
        });
        ...
        onDataChange(headers, selectedData.slice(1));
      } catch (error) {
        console.error('Error updating API data file:', error);
      }
    };
  }, [projectId, headers, tableData, onDataChange]);

  useEffect(() => {
    const timer = setTimeout(() => {
      if (hasUnsavedChanges && updateAPIDataFileRef.current) {
        updateAPIDataFileRef.current(selectedRows);
      }
    }, 5000); // Debounce for 5 seconds

    return () => clearTimeout(timer);
  }, [hasUnsavedChanges, selectedRows]);
