...
import GradientDonutChart from "../Charts/GradientDonut";

interface GradientDonutChartWithTableProps {
  color: string;
...
const GradientDonutChartWithTable: React.FC<GradientDonutChartWithTableProps> = ({
  color,
  titleAlignment,
  sourceName,
  sourceURL,
  ...
  return (
    <div>
      ...
      <DataTable onDataChange={handleDataChange} projectId={projectId} chartType={chartType} initialData={tableData.length === 0 ? initialData : undefined} />
    </div>
  );
};

export default GradientDonutChartWithTable;