...
interface TableDataTableProps {
  ...
  const TableDataTable: React.FC<TableDataTableProps> = ({
    onDataChange,
    projectId,
    chartType,
    initialData,
...
useEffect(() => {
  ...
  useEffect(() => {
    if (headers.length > 0) {
      const storedSelectedColumns = localStorage.getItem(`selectedColumns_${projectId}_${chartType}`);
      if (storedSelectedColumns) {
        setSelectedColumns(JSON.parse(storedSelectedColumns));
      } else {
        setSelectedColumns(new Array(headers.length).fill(true));
      }
    }
  }, [headers, projectId, chartType]);
  ...
  const handleColumnSelect = (index: number) => {
    ...
    localStorage.setItem(`selectedColumns_${projectId}_${chartType}`, JSON.stringify(newSelectedColumns));
  };

  const getFilteredData = () => {
    const filteredHeaders = headers.filter((_, index) => selectedColumns[index]);
  ...
  useEffect(() => {
    const { filteredHeaders, filteredData } = getFilteredData();
    onDataChange(filteredHeaders, filteredData);
                    ...
                    {headers.map((header, colIndex) => (
                      ...
                      >
                        <div className="flex items-center">
                          <input
                            type="checkbox"
                            checked={selectedColumns[colIndex]}
