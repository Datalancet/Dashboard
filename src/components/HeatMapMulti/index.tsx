...
import HeatMapMultiWithTable from "../HeatMapMultiwithTable";
...
const HeatMapMulti = () => {
  ...
  const [heatmapColors, setHeatmapColors] = useState<string[]>(() => {
    const storedColors = localStorage.getItem(`${projectId}_${chartType}_heatmapColors`);
    return storedColors ? JSON.parse(storedColors) : ["#3b82f6", "#10b981", "#ef4444"];
  });
  ...
  useEffect(() => {
    ...
    localStorage.setItem(`${projectId}_${chartType}_heatmapColors`, JSON.stringify(heatmapColors));
  ...
  }, [titleAlignment, sourceName, sourceURL, chartTitle, heatmapColors, xAxisTitle, yAxisTitle, showLegend, reversedYAxis, cellRadius, projectId, chartType, logoPosition, logoUrl]);
  ...
  const handleHeatmapColorChange = (index: number, color: string) => {
    const newColors = [...heatmapColors];
    newColors[index] = color;
    setHeatmapColors(newColors);
  };

  const handleAddSeries = () => {
    setHeatmapColors([...heatmapColors, "#000000"]);
  };

  const handleRemoveSeries = (index: number) => {
    const newColors = heatmapColors.filter((_, i) => i !== index);
    setHeatmapColors(newColors);
  };
  ...
  const handlePublish = () => {
    ...
    if (chartElement) {
      html2canvas(chartElement).then((canvas) => {
        ...
        link.download = "heatmap_multi.png";
        link.click();
      });
    }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <Breadcrumb pageName="Multi-Series Heat Map" />
      </div>
      
      <div className="flex">
        <div className="w-3/4" ref={chartRef}>
          <HeatMapMultiWithTable
    ...
    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg">
      {numericColumns.map((column) => (
        <div key={column} className="flex items-center p-2 hover:bg-gray-100">
          ...
          <div className="mb-4">
  <label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Heatmap Colors</label>
  {heatmapColors.map((color, index) => (
    <div key={index} className="flex items-center mb-2">
      <input
        type="color"
        value={color}
        onChange={(e) => handleHeatmapColorChange(index, e.target.value)}
        className="mr-2"
      />
      <span className="text-sm">Series {index + 1}</span>
      <button
