...
interface HeatMapChartwithTableProps {
  heatmapColor: string;
...
const HeatMapChartwithTable: React.FC<HeatMapChartwithTableProps> = ({
  heatmapColor,
  titleAlignment,
  sourceName,
  sourceURL,
  ...
  const aggregateData = () => {
    ...
    const yIndex = headers.indexOf(yAxisColumns[0]);
    const valueIndex = headers.indexOf(yAxisColumns[1]);
    ...
    const result = Object.entries(aggregated).map(([key, data]) => {
      ...
      if (aggregationMethod === 'average') {
        ...
        value = data.values[0]; // Fallback to first value
      }
...
export default HeatMapChartwithTable;