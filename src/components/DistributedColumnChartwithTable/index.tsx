...
import DistributedColumnChart from "../Charts/DistributedColumn";
import { useSearchParams } from 'next/navigation';
import Papa from "papaparse";

interface DistributedColumnChartWithTableProps {
  design: string;
  colors: string[];
...
const DistributedColumnChartWithTable: React.FC<DistributedColumnChartWithTableProps> = ({
  design,
  colors,
  gridVariation,
  xAxisPosition,
  ...
  const chartType = 'DistributedColumn';
  ...
  const handleDataChange = useCallback(async (newHeaders: string[], newData: any[]) => {
    ...
    for (let i = 0; i < newHeaders.length; i++) {
      if (newData.every(row => !isNaN(Number(row[i])))) {
        ...
        if (defaultY.length === 1) break; // For distributed column chart, we typically need only one Y-axis column
      }
    }
  ...
  const aggregateData = useCallback(() => {
    ...
    const yIndex = headers.indexOf(yAxisColumns[0]); // For distributed column, we use only the first Y-axis column
    ...
    return tableData.reduce((acc, curr) => {
      ...
      if (existingIndex > -1) {
        if (aggregationMethod === 'sum') {
          acc[existingIndex][1] = (parseFloat(acc[existingIndex][1]) + parseFloat(curr[yIndex])).toString();
...
export default DistributedColumnChartWithTable;