...
import StackedColumnChartWithTable from "../StackedColumnChartwithTable";
...
const StackedColumnChart = () => {
  ...
  const chartType = 'StackedColumn'; // This identifies the chart type
  ...
  const [colors, setColors] = useState(() => {
    const savedColors = localStorage.getItem(`${projectId}_${chartType}_colors`);
    return savedColors ? JSON.parse(savedColors) : ["#3b82f6", "#10b981", "#f59e0b"];
  });

  useEffect(() => {
    localStorage.setItem(`${projectId}_${chartType}_colors`, JSON.stringify(colors));
  }, [colors, projectId, chartType]);
  ...
  }, [design, isGridMode, gridVariation, isLabelStyle, labelPosition, xAxisPosition, yAxisPosition, titleAlignment, sourceName, sourceURL, valuesPosition, chartTitle, color, seriesNames, xAxisTitle, yAxisTitle, projectId, chartType, logoPosition, logoUrl,showPercentages]);
  ...
  const handlePublish = () => {
    const chartContainer = document.querySelector('.col-span-12');
    if (chartContainer) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <Breadcrumb pageName="Stacked Column Chart" />
      </div>
      
      <div className="flex">
        <div className="w-3/4" ref={chartRef}>
        <StackedColumnChartWithTable
            design={design}
            colors={colors}
        ...
        <div className="mb-4">
            {seriesNames.map((name: string | number | readonly string[] | undefined, index: React.Key | null | undefined) => (
 ...
 <div className="mb-4">
            <label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Chart Colors</label>
            {colors.map((color: string | number | readonly string[] | undefined, index: React.Key | null | undefined) => (
