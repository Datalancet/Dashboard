...
import ReversedChartWithTable from "../ReversedChartwithTable";
...
const ReversedBar = () => {
  ...
  const chartType = 'reversed-bar'; // This identifies the chart type
  ...
  useEffect(() => {
    if (xAxisColumn) {
      localStorage.setItem(`${projectId}_${chartType}_xAxisColumn`, xAxisColumn);
    }
  }, [xAxisColumn, projectId, chartType]);

  useEffect(() => {
    if (yAxisColumns.length > 0) {
      localStorage.setItem(`${projectId}_${chartType}_yAxisColumns`, JSON.stringify(yAxisColumns));
    }
  }, [yAxisColumns, projectId, chartType]);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <Breadcrumb pageName="Reversed Bar" />
      </div>
      
      <div className="flex">
        <div className="w-3/4" ref={chartRef}>
          <ReversedChartWithTable
            design={design}
            color={color}
